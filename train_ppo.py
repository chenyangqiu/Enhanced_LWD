...
parser.add_argument(
    ...
    type=str,
    default="data"  # <== 设置默认值（根据你本地的数据路径修改）
)

parser.add_argument(
    "--device",
    help="id of gpu device to use",
    type=str,  # 改为 str 类型（比如 "cuda:0"）
...
# construct datasets
datasets = {
    "train": get_er_15_20_dataset("train", os.path.join(base_data_dir, "train")),
    ...
    for g in data_loaders[mode]:
        ...
        while True:
            ...
            if torch.all(done).item():
                ...
                sol = info['sol']
                if sol.dim() == 2:
                    cum_eval_sol += sol.max(dim=1)[0].sum().cpu()
                else:
                    cum_eval_sol += sol.float().sum().cpu()
...
for update_t in range(max_update_t):
    ...
    for step_t in range(max_rollout_t):
        ...
        if torch.all(done).item():
            # print("info['sol'].shape:", info['sol'].shape)

            sol = info['sol']
            if sol.dim() == 2:
                avg_sol = sol.max(dim=1)[0].mean().cpu()
            else:
                avg_sol = sol.float().mean().cpu()
    ...
    print("update_t:", update_t)
    print("rollout.obs shape:", rollout.obs.shape)
    print("rollout.g.batch_size:", rollout.g.batch_size)
