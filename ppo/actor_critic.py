...
class ActorCritic(nn.Module):
    ...
    def get_masks_idxs_subg_h(self, ob, g):
        ...
        # if node_mask.sum() == 0:
        #     raise ValueError("⚠️ node_mask 全为 0，当前 batch 无有效节点。")
    ...
    def act_and_crit(self, ob, g):
        ...
        # compute logits to get action
        if flatten_node_idxs.numel() == 0 or flatten_subg_node_idxs.numel() == 0:
            print("⚠️ Warning: empty node index in act_and_crit, skipping this batch")
            dummy_shape = (num_nodes * batch_size,)
            return (
                torch.zeros((num_nodes, batch_size), dtype=torch.long, device=self.device),  # action
                torch.zeros((num_nodes, batch_size), device=self.device),  # log prob
                torch.zeros((batch_size,), device=self.device)  # value_pred
            )
    ...
    def evaluate_batch(self, ob, g, action):
        ...
        # compute logits to get action
        if flatten_node_idxs.numel() == 0 or flatten_subg_node_idxs.numel() == 0:
            print("⚠️ Warning: empty node index in evaluate_batch, skipping this batch")
            dummy_shape = (num_nodes * batch_size,)
            return (
                torch.zeros((num_nodes, batch_size), device=self.device),  # log_probs
                torch.tensor(0.0, device=self.device),  # avg_entropy
                torch.zeros((batch_size,), device=self.device),  # value_preds
                node_mask
            )
